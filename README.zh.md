...
Minecraft 服务器的脚本化压测 / 观测包装器。接管 vanilla / Fabric / Paper 服务器的 stdin/stdout，用一种叫 `.mcb` 的小脚本语言驱动服务器按剧本执行，把采集到的指标导出为带时间戳的 CSV + JSON。

*[English](README.md)*

## 它是做什么的

`observer` 把 Minecraft 服务器进程包起来，按脚本执行剧本：
